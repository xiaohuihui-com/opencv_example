...
if __name__ == '__main__':
    ...
    '''
    image = cv2.imread('../resource/images/2.png',1)
    dst1 = Img_findContours(image,1)
    dst2 = Img_findContours(image,2)
    dst3 = Img_findContours(image,3)
    dst4 = Img_findContours(image,4)
    dst5 = Img_findContours(image,5)
    showImg(dst1)
    showImg(dst2)
    showImg(dst3)
    showImg(dst4)
    ...
    titles = ['imgGray','1','2','3','4','5']
    plt.figure('morphology deal')
    for i in range(6):
        plt.subplot(2,3,i+1) #行，列，第几个位置
        plt.imshow(images[i]) #灰度显示，默认rgb显示
